...
<head>
  ...
  <style>
    ...
    .step-new:not(:last-child)::after {
      content:''; position:absolute; top: 30%; left:67%;
      width:19%; height:1px; background:#ddd; z-index:0;
    }
    .step-circle {
    ...
    .step-circle.idle    { background:#eee; color:#999; border:1px solid #ddd; }
    .step-label { font-size:11px; color:#888; margin-top:4px; display:block; }

    .form-step { display:none; }
    .form-step.active { display:block; }

    .role-cards { display:flex gap:10px; margin-top:8px; }
    .role-card {
      border:1.5px solid #e0e0e0; border-radius:8px;
      padding:16px 8px; text-align:center; cursor:pointer; transition:.15s;
    }
    .role-card:hover  { border-color:#aaa; background:#f8f8f8; }
    .role-card.selected { border:2px solid #1a7a4a; background:#eaf5ef; }
    .role-card .icon  { font-size:24px; margin-bottom:6px; }
    .role-card .label { font-size:13px; font-weight:600; }
    .role-card .sub   { font-size:11px; color:#777; margin-top:2px; }

    .upload-box {
      border:1px dashed #bbb; border-radius:8px;
      padding:12px 14px; background:#fafafa; margin-top:10px;
    }
    .upload-box label { font-weight:600; font-size:13px; color:#333; margin-bottom:4px; display:block; }
    .upload-hint { font-size:12px; color:#888; margin-bottom:8px; }
    .badge-req { background:#ffe4e4; color:#c0392b; font-size:10px; padding:2px 6px; border-radius:4px; margin-left:6px; }

    .info-box { background:#e8f5f0; border-left:3px solid #1a7a4a; border-radius:0 6px 6px 0; padding:10px 14px; font-size:13px; color:#155a38; margin-top:12px; }

    .nav-row { display:flex; justify-content:space-between; margin-top:1.5rem; gap:10px; }
    .nav-row .btn-back { flex:0 0 auto; }
    .nav-row .btn-next { flex:0 0 130px; }

    .field-error { font-size:12px; color:#e74c3c; margin-top:3px; display:none; }
  </style>
</head>
<body>
...
function updateBar(step) {
  for (let i = 1; i <= 3; i++) {
    ...
    if (i < step) { c.className = 'step-circle done'; c.textContent = '✓'; }
    else if (i === step) { c.className = 'step-circle active'; c.textContent = i; }
    else { c.className = 'step-circle idle'; c.textContent = i; }
  }
}
...
function validateStep1() {
  let ok = true;
  const show = (id, vis) => { document.getElementById(id).style.display = vis ? 'block' : 'none'; };
  const name  = document.getElementById('name').value.trim();
  const email = document.getElementById('email').value.trim();
  const pass  = document.getElementById('password').value;
  const conf  = document.getElementById('confirm_password').value;
  show('err-name',    !name);                         if (!name) ok = false;
  show('err-email',   !email || !email.includes('@')); if (!email || !email.includes('@')) ok = false;
  show('err-pass',    pass.length < 8);               if (pass.length < 8) ok = false;
  show('err-confirm', pass !== conf);                 if (pass !== conf) ok = false;
  return ok;
}
